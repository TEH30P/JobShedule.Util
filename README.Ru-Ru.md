...
Параметры: 
- \#0 : **`!обяз`**, `[string]` - путь к корневой папке, куда надо складывать логи. Если праметр не указан, то командлеты записи в лог будут создавать файлы в директории `%temp%`.

# Команды логирования 
`*-SJLog*` (`~SJLog~*`)

Логи пишутся в формате [YAML](http://yaml.org/). Для работы лога нужно указать папку, куда логировать; она указывается в первом параметра загрузки модуля или через вызов команды "`~SJLog~Dir~Set <путь-к-папке> -fRoot`", по-умолчанию используется `%temp%`. Алгоритм формирования сообщений и записи их в файлы оптимизирован на минимальное использование дополнительных буфферов памяти: чтоб увеличить вероятность успешной записи в лог, в ситуациях когда хост испытывает критические нагрузки (например недостаток памяти). Так же алгоритм формирования сообщения в лог реализован так, что в случае сбоя записи сообщения, целостность логической структуры файла лога не будет нарушена.
...
- \#1 `iLogAt` : **`игнор`**, `[datetime]` - Устаревший параметр. Сохранён для совместимости.
...
## Команда `Set-SJLogDate` (`~SJLog~Date~Set`)
Изменение настроек логирования: дата открытия лога, которая указывается в имени файла лога.

Параметры:
- \#0 `iValue` : **`!обяз`**, `[string]` - Дата открытия лога. По-умолчанию: текущая дата (`Get-Date`).
